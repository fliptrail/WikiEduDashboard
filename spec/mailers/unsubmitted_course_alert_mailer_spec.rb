...
describe UnsubmittedCourseAlertMailer do
  describe '#send_email' do
    ...
    let(:alert) { create(:unsubmitted_course_alert, target_user: instructor, course: course) }
    ...
    context 'user with a real name' do
      ...
      it 'sends an email to the course instructor' do
        ...
        expect(mail.body).to include(instructor.real_name)
      end
    end
    ...
    context 'user with only a username' do
      ...
      it 'sends an email to the course instructor' do
        ...
        expect(mail.subject).to eq(subject)
        expect(mail.to).to eq([instructor.email])
        expect(mail.body).to include(instructor.username)
      end
    end
  end
end
